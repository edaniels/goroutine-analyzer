...
import {parseGoroutines, Goroutine} from './DumpParser';
const fs = require('fs');
...
`;
  ...
  let goroutines: any[] = [];
  parseGoroutines(input, goroutine => {
    goroutines.push(goroutine);
  });

  expect(goroutines).toHaveLength(5);

  expect(goroutines[0].id).toBe(11044395);
  expect(goroutines[0].state).toBe('running');
  expect(goroutines[0].timeMinutes).toBe(0);
  expect(goroutines[0].lines).toHaveLength(10);

  expect(goroutines[1].id).toBe(1);
  expect(goroutines[1].timeMinutes).toBe(2);
  expect(goroutines[1].state).toBe('semacquire');
  expect(goroutines[1].lines).toHaveLength(16);

  expect(goroutines[2].id).toBe(5);
  expect(goroutines[2].timeMinutes).toBe(2);
  expect(goroutines[2].state).toBe('syscall');
  expect(goroutines[2].lines).toHaveLength(6);

  expect(goroutines[3].id).toBe(11052961);
  expect(goroutines[3].timeMinutes).toBe(0);
  expect(goroutines[3].state).toBe('select');
  expect(goroutines[3].lines).toHaveLength(4);

  expect(goroutines[4].id).toBe(1985);
  expect(goroutines[4].timeMinutes).toBe(299);
  expect(goroutines[4].state).toBe('chan receive');
  expect(goroutines[4].lines).toHaveLength(10);
});
